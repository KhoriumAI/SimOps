...
class CFDPDFReportGenerator:
    """
    Generates professional CFD analysis reports including Drag/Lift coefficients and flow visualizations.
    """
    ...
    def generate(self, 
        ...
        """
        Generate CFD PDF report.
        
        Args:
            job_name: Name of the simulation job
            output_dir: Directory to save the PDF
            data: Dictionary containing simulation results (Cd, Cl, Re, etc.)
            image_paths: List of paths to plot images to embed
            
        Returns:
            Path to the generated PDF file
        """
        pdf_path = output_dir / f"{job_name}_cfd_report.pdf"
        ...
        doc = SimpleDocTemplate(
            ...
            topMargin=20*mm,
            bottomMargin=20*mm,
            title=f"SimOps CFD Report - {job_name}"
        )
        ...
        # 1. Header
        story.append(Paragraph("SimOps CFD Analysis", title_style))
        ...
        grade = "CONVERGED" if converged else "DIVERGED"
        grade_color = self.colors['success'] if converged else self.colors['danger']
        ...
        # 3. Key Metrics Table
        # Default keys for CFD
        reynolds = data.get('reynolds_number', 'N/A')
        cd = data.get('drag_coefficient', 'N/A')
        cl = data.get('lift_coefficient', 'N/A')
        
        # Format if float
        if isinstance(reynolds, (int, float)): reynolds = f"{reynolds:.1f}"
        if isinstance(cd, float): cd = f"{cd:.4f}"
        if isinstance(cl, float): cl = f"{cl:.4f}"
        ...
        # 4. Visualizations
        story.append(Paragraph("Flow Visualization", heading_style))

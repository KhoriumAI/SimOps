...
class ModernMeshGenGUI(QMainWindow):
    ...
    def create_left_panel(self):
        ...
        self.viz_metric_combo.addItems(["SICN (Min)", "Gamma", "Skewness", "Aspect Ratio", "Jacobian"])
...
# --- VISUALIZATION HANDLERS ---
    ...
    def update_quality_data_ranges(self, per_element_quality, per_element_gamma, per_element_skewness, per_element_aspect_ratio):
        ...
        if per_element_quality:
            ...
            # Also set Jacobian range from same data (hex meshes use Jacobian as primary quality)
            self.quality_data_ranges['Jacobian'] = (min(quality_values), max(quality_values))
    ...
    def on_intermediate_update(self, phase: str, path: str):
        """Handle intermediate mesh updates from worker"""
        try:
            # We must normalize path
            path = str(Path(path).absolute())
            print(f"[GUI] Received intermediate update: {phase} -> {path}")
            self.add_log(f"Displaying intermediate result: {phase}...")
            
            # Special handling per phase
            if phase == 'surface':
                success = self.vtk_viewer.load_mesh_file(path)
                if success:
                    self.current_process_label.setText(f"Visualizing Surface Mesh")
                    # Disable cross section for surface mesh (it looks bad/empty inside)
                    if self.crosssection_enabled.isChecked():
                         self.crosssection_enabled.setChecked(False)
                    self.crosssection_enabled.setEnabled(False)
                    
            elif phase == 'coacd':
                success = self.vtk_viewer.load_mesh_file(path)
                if success:
                    self.current_process_label.setText(f"Visualizing Convex Decomposition")
                    self.crosssection_enabled.setEnabled(True)
                    
            elif phase == 'unstructured':
                success = self.vtk_viewer.load_mesh_file(path) 
                if success:
                    self.current_process_label.setText(f"Visualizing Unstructured Mesh")
                    self.crosssection_enabled.setEnabled(True)
            
            else:
                 # Generic fallback
                 self.vtk_viewer.load_mesh_file(path)

        except Exception as e:
            print(f"[GUI] Error loading intermediate mesh: {e}")
            self.add_log(f"Intermediate display error: {e}")
